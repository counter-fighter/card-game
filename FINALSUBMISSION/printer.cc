...
void Printer::printLowerBoarder(bool showText, bool showWindow) {
    ...
    if (showWindow && enableGraphics) window->drawString(lowerBoarder);
}
...
void Printer::Printer::printCardsWithBoarder(bool showText, bool showWindow) {
    for (int line = 0; line < cardHeight; line++) { // Printing one line at a time for maxCardPerRow
        ...
        if (showWindow && enableGraphics) window->drawString(cardLine);
    } 
    cards.clear();
}
...
void Printer::printHelp() {
    ...
    cout << "          inspect minion [e] -- View a minion\'s card and all enchantments on that minion. Optionally"
         << " inspect an opponent's minion by specifying e for enemy." << endl;
